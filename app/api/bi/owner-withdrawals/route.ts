...
export async function GET(): Promise<Response> {
  try {
    return Response.json({ data: await service.listOwnerWithdrawals() });
  } catch (error) {
...
export async function POST(request: Request): Promise<Response> {
  try {
    return Response.json({ data: await service.createOwnerWithdrawal(await readJsonObject(request)) }, { status: 201 });
  } catch (error) {
    return businessIntelligenceErrorResponse(error);
  }
}
